...
- Qwen models are allowlisted through OpenCode Go at startup. No hidden provider or Western-model fallback is permitted.
...
Requirements: Node.js 22–25, Docker, an OpenCode Go API key, and a Telegram bot token.

1. Copy `.env.example` to `.env`.
2. Fill `TELEGRAM_BOT_TOKEN`, `OPENCODE_API_KEY`, and `OPENCODE_SERVER_PASSWORD`. Docker Compose provides the local STT endpoint.
...
All external model calls go through OpenCode Go. Voice transcription stays inside the deployment on the local `faster-whisper` container.
...
5. Restore the prior environment only if it keeps the `opencode-go/qwen…` allowlist and local STT endpoint.
